...
async function insertSessionData(session_id, user_name, user_id, psy_id, session_date, session_time) {
  const client = await pool.connect();

  try {
    await client.query('BEGIN'); // Start a transaction

    const query = 'INSERT INTO sessions (session_id, user_name, user_id, psy_id, session_date, session_time) VALUES ($1, $2, $3, $4, $5, $6)';
    const values = [session_id, user_name, user_id, psy_id, session_date, session_time];
...
module.exports = { pool , insertUserData, AuthenticateData, insertPsyData, insertSessionData, AuthenticatePsyData, insertBlogData};
